...
},{"fft.js":10,"next-pow-2":11}],8:[function(require,module,exports) {
'use strict';
...
var getPitchFromNote = exports.getPitchFromNote = function getPitchFromNote(note) {
  var noteIndex = 0;
  notes.some(function (item, index) {
    if (item === note.name) {
      noteIndex = index;
      return true;
    }
  ...
  var offset = (noteIndex - 9 - notesNumber * (4 - note.octave)) / notesNumber;
  return Math.round(Math.pow(2, offset) * baseFrequency * 100) / 100;
};

var getNoteFromPitch = exports.getNoteFromPitch = function getNoteFromPitch(pitch) {
...
exports.ukulele = exports.dropC = exports.dropD = exports.openG = exports.openD = exports.openC = exports.standard = exports.chromatic = undefined;

var _extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; };

var _onverters = require('./\u0441onverters');

var mapStrings = function mapStrings(item) {
  return _extends({}, item, { pitch: (0, _onverters.getPitchFromNote)(item) });
};

var parse = function parse(pitch) {
  var note = (0, _onverters.getNoteFromPitch)(pitch);
  if (!note) {
    return false;
  }

  if (this.strings.some(function (_ref) {
    var name = _ref.name,
        octave = _ref.octave;
  ...
  var _strings$reduce = this.strings.reduce(function (acc, item) {
    ...
    return acc;
      ...
      name = _strings$reduce.name,
      octave = _strings$reduce.octave;
...
var chromatic = exports.chromatic = {
  name: 'Chromatic',
  parse: function parse(pitch) {
    return (0, _onverters.getNoteFromPitch)(pitch);
  }
};

var standard = exports.standard = {
  name: 'Standard',
  strings: [{ name: 'E', octave: 2 }, { name: 'A', octave: 2 }, { name: 'D', octave: 3 }, { name: 'G', octave: 3 }, { name: 'B', octave: 3 }, { name: 'E', octave: 4 }].map(mapStrings),
  parse: parse
};

var openC = exports.openC = {
  name: 'Open C',
  strings: [{ name: 'C', octave: 2 }, { name: 'G', octave: 2 }, { name: 'C', octave: 3 }, { name: 'G', octave: 3 }, { name: 'C', octave: 4 }, { name: 'E', octave: 4 }].map(mapStrings),
  parse: parse
};

var openD = exports.openD = {
  name: 'Open D',
  strings: [{ name: 'D', octave: 2 }, { name: 'A', octave: 2 }, { name: 'D', octave: 3 }, { name: 'F#', octave: 3 }, { name: 'A', octave: 3 }, { name: 'D', octave: 4 }].map(mapStrings),
  parse: parse
};

var openG = exports.openG = {
  name: 'Open G',
  strings: [{ name: 'D', octave: 2 }, { name: 'G', octave: 2 }, { name: 'D', octave: 3 }, { name: 'G', octave: 3 }, { name: 'B', octave: 3 }, { name: 'D', octave: 4 }].map(mapStrings),
  parse: parse
};

var dropD = exports.dropD = {
  name: 'Drop D',
  strings: [{ name: 'D', octave: 2 }, { name: 'A', octave: 2 }, { name: 'D', octave: 3 }, { name: 'G', octave: 3 }, { name: 'B', octave: 3 }, { name: 'E', octave: 4 }].map(mapStrings),
  parse: parse
};

var dropC = exports.dropC = {
  name: 'Drop C',
  strings: [{ name: 'C', octave: 2 }, { name: 'G', octave: 2 }, { name: 'C', octave: 3 }, { name: 'F', octave: 3 }, { name: 'A', octave: 3 }, { name: 'D', octave: 4 }].map(mapStrings),
  parse: parse
};

var ukulele = exports.ukulele = {
  name: 'Ukulele',
  strings: [{ name: 'G', octave: 4 }, { name: 'C', octave: 4 }, { name: 'E', octave: 4 }, { name: 'A', octave: 4 }].map(mapStrings),
  parse: parse
};
},{"./сonverters":8}],6:[function(require,module,exports) {
'use strict';
...
var stringsWrapperElement = document.querySelector('.strings');
...
var drawActiveString = function drawActiveString(name) {
  stringsWrapperElement.childNodes.forEach(function (item) {
...
var _tunings = require('./tunings');

var tunings = _interopRequireWildcard(_tunings);
...
var tuningSelectElement = document.querySelector('.tunings-list');
var stringsListElement = document.querySelector('.strings');

var overlayElement = document.querySelector('.overlay');
var tunerShowBtnElement = document.querySelector('.tuner-show-btn');

var defaultTuning = 'standard';
var activeTuning = tunings[defaultTuning];

tuningSelectElement.innerHTML = Object.keys(tunings).map(function (key) {
  return '<option value=' + key + ' ' + (key === defaultTuning && 'selected') + '>' + tunings[key].name + '</option>';
}).join('');

var handleTuningChange = function handleTuningChange() {
  ...
  stringsListElement.innerHTML = activeTuning.strings.map(function (_ref) {
    var name = _ref.name,
        octave = _ref.octave;
    return '<div class="strings-item" data-note="' + (name + octave) + '">\n        <div class="string-lightbulb"></div>\n        <span class="string-name">' + (name + octave) + '</span>\n      </div>';
  }).join('');
};

var handleGetAudioInput = function handleGetAudioInput(stream) {
  ...
  setInterval(function () {
    ...
    if (clarity > 0.9) {
      (0, _render2.default)(activeTuning.parse(pitch));
    }
  }, 100);
};

tunerShowBtnElement.addEventListener('click', function () {
...
},{"pitchy":9,"./tunings":5,"./render":6}],12:[function(require,module,exports) {
...
//# sourceMappingURL=/dist/ec4d261ca768700929a19219f0e9e071.map